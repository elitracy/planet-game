...
func CreatePlanet(name string, position engine.Position, pop, num_farms, num_mines, num_solar_grids int) Planet {
	planet := Planet{
		...
		Population:           pop,
		PopulationGrowthRate: game.POPULATION_GROWTH_PER_PULSE,
		Resources: Resources{
			Food: resources.Food{
				Quantity:        pop * game.FOOD_PER_PERSON * game.NUM_DAYS_FED,
...
func (p *Planet) TickResources() {
	...
	requiredFood := p.Population * game.FOOD_PER_PERSON

	if currentFood >= requiredFood {
		p.Food.Quantity -= requiredFood

		currentFood = p.Food.Quantity
		addPopRequiredFood := game.FOOD_PER_PERSON * p.PopulationGrowthRate

		if currentFood >= addPopRequiredFood {
			p.Food.Quantity -= game.FOOD_PER_PERSON * p.PopulationGrowthRate
